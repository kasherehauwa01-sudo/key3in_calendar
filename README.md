...
Compose ждёт успешной внутренней проверки backend перед запуском frontend. Поэтому
после пересборки прокси не начинает принимать запросы к API, пока миграции не
завершены и Uvicorn ещё не готов отвечать; это предотвращает временный `502` в
сценарии обновления.
