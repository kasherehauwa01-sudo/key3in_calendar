...
export function SearchView({ query, setQuery, online, userId, onClose, onSelect, onError }: SearchViewProps) {
  ...
  return <Box sx={{flex:1,minHeight:0,overflowY:'auto'}}>
    ...
    <List>{results.map(note => <ListItemButton key={note.date} onClick={() => onSelect(note)} sx={{ borderRadius: 3, mb: 1, bgcolor: 'background.paper' }}>
